    ...
    fn eval_cast(&self, value: Value, target: &Type) -> Result<Value, InterpretError> {
        match target {
            ...
            Type::Bool => match value {
                Value::Bool(b) => Ok(Value::Bool(b)),
                Value::Int(int) => Ok(Value::Bool(!int.value.is_zero())),
                Value::Field(f) => Ok(Value::Bool(f != FieldElement::zero())),
                other => Err(InterpretError::Type(format!(
...
mod semantics_tests {
    ...
    fn shift_right_logical_and_arithmetic() {
        ...
        assert_eq!(
            as_i128(eval(BinaryOpKind::ShiftRight, int(true, 8, -8), int(false, 8, 1)).unwrap()),
            -4
        );
